...
const MyClasses = () => {
    ...
    useEffect(() => {
        axioSecure.get(`/classes/${currentUser?.email}`).then(res => setClasses(res.data)).catch(err => console.log(err))
    }, [isLoading]);
    return (
        <div>
            <div className="my-9">
            ...
            <div>
                {
                    classes.length === 0 ? <div className='text-center text-2xl font-bold mt-10'>You have not added any classes yet</div> :
                        <div>
                            {
                                ...
                                <div key={index} className='mb-5 hover:ring ring-secondary duration-200 focus:ring rounded-lg' >
                                    <div className='bg-white flex rounded-lg gap-8 shadow p-4' >
                                        <div>
                                            <img src={cls.image} alt="no" className='max-h-[200px] rounded-md  max:w-[300px]' />
                                        </div>
                                        <div className="w-full">
                                            ...
                                            <div className='flex items-center justify-between'>
                                                <div>
                                                    ...
                                                    <h1 className=" text-secondary my-2">
                                                        <span className="text-black">Total Seats</span>:{" "}{cls.avialabelseats}
                                                    </h1>
                                                    <h1 className="text-secondary my-2">
